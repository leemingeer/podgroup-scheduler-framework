...
import (
	...
	"github.com/leemingeer/podgroup-scheduler-framework/pkg/util"
)
...
type Manager interface {
	...
	GetPodGroup(*corev1.Pod) (string, *schedulingv1.PodGroup)
...
func (pgMgr *PodGroupManager) PostBind(ctx context.Context, pod *corev1.Pod, nodeName string) {
	...
	if pgCopy.Status.Scheduled >= pgCopy.Spec.MinMember {
		pgCopy.Status.Phase = schedulingv1.PodGroupScheduled
	} else {
		pgCopy.Status.Phase = schedulingv1.PodGroupScheduling
...
func (pgMgr *PodGroupManager) PatchPodGroup(pgName string, namespace string, patch []byte) error {
	...
	_, err := pgMgr.pgClient.SchedulingV1().PodGroups(namespace).Patch(context.TODO(), pgName,
		types.MergePatchType, patch, metav1.PatchOptions{})
	return err
}

// GetPodGroup returns the PodGroup that a Pod belongs to from cache.
func (pgMgr *PodGroupManager) GetPodGroup(pod *corev1.Pod) (string, *schedulingv1.PodGroup) {
	pgName := util.GetPodGroupLabel(pod)
	if len(pgName) == 0 {
		return "", nil
	}
